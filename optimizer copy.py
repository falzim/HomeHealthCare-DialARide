...
def solve(I_total,              # set of all customers and Medical center
    ...
    # how many cars c are used parallel:
    cars_used = model.addVar(name='cars_used', vtype='c', lb=0) 
    ...
    if allow_delay_clients:
        model.setObjective(
            quicksum(Dt['MCd',s] - Pt['MC',s] for s in S) * 16/60 # staff cost
            + cars_used * 50 # fixed cost for car being used
            + quicksum(car_travel[c] for c in range(number_of_vehicles)) * 13/60 # cost for driver
