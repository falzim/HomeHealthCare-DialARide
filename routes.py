...
def solve(I_total,              # set of all customers and Medical center
    ...
    if allow_overtime:
        ...
        model.addConstrs(Dt['MCd',s] - Pt['MC',s] <= 480 + overtime[s] for s in staff)  
    else:
        # limit working hours of nurses to eight hours:
        model.addConstrs(Dt['MCd',s] - Pt['MC',s] <= 480 for s in staff)    
    for s in staff: 
            ...
            for j in I_total:
                if i != j:
                    for r in range(allowed_routes):
                        ...
                        model.addConstr(quicksum(o[i,j,s,r] for s in staff) + quicksum(op[i,j,l,r] for l in I0) <= vehicle_capacity)
                                    ...
                                    if i != j:
                                        if round(X[i,j,r].getAttr('X')) == 1:
                                            ...
                                            new_row =   {'Current Node': i,
                                                        'Next Node': j,
                                                        'Time': round(t[i,r].getAttr('X')),
                                                        'Used Capacity (leaving the node)': sum(round(o[i,j,s,r].getAttr('X')) for s in staff) 
